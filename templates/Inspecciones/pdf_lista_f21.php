<?php
/**
 * Lista de inspección — F-20 REV.01 (Dompdf)
 * Mapeo checklist ↔ BD: ver bloque $secciones y comentarios «Llantas por…».
 *
...
// Esta plantilla es solo F-21.
$tipoFormulario = 'F21_AUTOBUS';
$formularioNumero = 'F-21';
$formularioRev = 'F-21 REV.01';
$esCabina   = true;
$esRemolque = false;
$esDolly    = false;
$esCamion   = false;
...
/* Oficial F-21: siempre imprime 1/2 dir + 3/4 + 5/6 + 7 + 8 (texto F-21_AUTOBUS.txt). */
$tireRowsF21 = static function (array $spec) use ($LG, $L): array {
    $n = (int)$spec['gn'];
    $pos = $spec['pos'] ?? null;
    $lg = $pos !== null
        ? static fn (string $f): ?string => $L($n, $pos, $f) ?? $LG($n, $f)
    ...
    $filas = [
        ...
        ['nom'=>'77','d'=>'','v'=>'✓','m'=>'✓','txt'=>'COSTADOS: CONDICION ABULATAMIENTO MAS DE 9,5mm,DAÑO (CORTES), CONCORDANCIA, INDICACIONES', 'val'=>$lg('costados')],
        ['nom'=>'77','d'=>'','v'=>'✓','m'=>'✓','txt'=>'PRESION', 'val'=>$lg('presion_cumple')],
        ['nom'=>'78','d'=>'','v'=>'✓','m'=>'', 'txt'=>$spec['rin'], 'val'=>$lg('rin_condicion')],
        ['nom'=>'78','d'=>'','v'=>'✓','m'=>'', 'txt'=>'CONDICION, CONCORDANCIA, BOQUILLA VALVULA DE AIRE', 'val'=>$lg('rin_condicion')],
        ['nom'=>'78','d'=>'','v'=>'✓','m'=>'', 'txt'=>'SUJETADORES, TUERCAS BIRLOS', 'val'=>$lg('rin_sujetadores')],
        ['nom'=>'78','d'=>'','v'=>'✓','m'=>'', 'txt'=>'RIN DE ARTILLERIA, PIEZAS MULTIPLES, CONDICION, ABRAZADERAS, ANILLOS DE SEGURIDAD', 'val'=>\App\Validation\InspeccionMexico::valorRinArtilleria($lg('rin_condicion'), $lg('rin_artilleria'))],
    ];
...
$gruposLlantaF21 = [
    // Checklist oficial F-21: 1/2, 3/4, 5/6, 7 y 8 sueltos (sin bloque 9/10).
    // La captura B3 igual tiene 10 llantas; 9/10 van a la tabla complementaria del pie.
    ['gn' => 1, 'dir' => true,  'titulo' => 'LLANTA 1/2 DIRECCIONAL PROFUNDIDAD MINIMO DE 3,2 mm', 'rin' => 'RIN 1/2 DIRECCIONAL DE DISCO', 'pos' => null],
    ['gn' => 3, 'dir' => false, 'titulo' => 'LLANTA 3/4 EXTERIOR  ESPESOR MINIMO 1,6 mm', 'rin' => 'RIN 3/4 DE DISCO', 'pos' => null],
    ['gn' => 5, 'dir' => false, 'titulo' => 'LLANTA 5/6 INTERIOR ESPESOR MINIMO 1,6 mm', 'rin' => 'RIN 5/6  DE DISCO', 'pos' => null],
    ['gn' => 7, 'dir' => false, 'titulo' => 'LLANTA 7 DISCO ESPESOR MINIMO 1,6 mm', 'rin' => 'RIN 7 DE DISCO', 'pos' => null],
    ['gn' => 8, 'dir' => false, 'titulo' => 'LLANTA 8  ESPESOR MINIMO 1,6 mm', 'rin' => 'RIN 8  DE DISCO', 'pos' => null],
];
$llDelanteras = [];
$llTraseras = [];
$maxLlActivas = (int)(\App\Validation\TipoVehiculoRequisitos::definicion($tipoVehCod)['llantas'] ?? 0);
foreach ($gruposLlantaF21 as $spec) {
    if ($maxLlActivas > 0 && (int)$spec['gn'] > $maxLlActivas) {
        continue;
    }
    $rows = $tireRowsF21($spec);
    if ($spec['dir']) {
...
if ($tipoFormulario === 'F19_REMOLQUE') {
    // Orden oficial F-19 REV.01 (webroot/camposTXT/F-19_REMOQLUE.txt / f-19.pdf).
    $add($secciones, 'LUCES', $secLucesRemolque);
    $add($secciones, 'PARTE TRASERA', $secParteTraseraRemolque);
    $add($secciones, 'LLANTAS', $llTraseras);
    $add($secciones, 'SUSPENSION', $suspRemolquePdf);
    $add($secciones, 'FRENOS NEUMATICOS', $secNeuReducido);
    $add($secciones, 'FRENOS ELECTRICOS', [
        ['nom'=>'63','d'=>'','v'=>'✓','m'=>'','txt'=>'FRENOS ELECTRICOS (RETARDADORES)', 'val'=> $fr?->frenos_electricos_ret],
    ]);
    $add($secciones, 'CHASIS', [
        ['nom'=>'57','d'=>'','v'=>'✓','m'=>'','txt'=>'CUERPO DEL TANQUE', 'val'=> $car?->cuerpo_tanque],
    ...
    $add($secciones, 'LLANTAS', $llTraseras);
    $add($secciones, 'SUSPENSIÓN', $secSuspTrasera);
    $add($secciones, 'VIGAS Y MONTAJE DEL CHASIS', $secVigas);
    $add($secciones, 'FRENOS NEUMÁTICOS', $secNeuDolly);
    $add($secciones, 'SISTEMA DE ACOPLAMIENTO', $secAcoplamiento);
} elseif ($tipoFormulario === 'F21_AUTOBUS') {
    // Orden oficial F-21 REV.01 (F-21_AUTOBUS.txt). Solo esta plantilla.
    $suspTraseraF21 = [
      ['nom'=>'14','d'=>'','v'=>'✓','m'=>'','txt'=>'MUELLES, GRILLETES, SOPORTES EN EXTREMO DE MUELLES, BUJES/PERNOS, EQUILIBRADORES,TOPES DE IMPACTO', 'val'=> $su?->muelles],
      ['nom'=>'15','d'=>'','v'=>'✓','m'=>'','txt'=>'SUSPENSIÓN DE BARRA DE TORSION: BUJES Y PASADORES DE GRILLETE, BARRA DE TORCION, SOPORTES DE MONTAJE, ELEMENTOS DE SUJECION', 'val'=> $su?->barra_torsion],
      ['nom'=>'21','d'=>'','v'=>'✓','m'=>'','txt'=>'AMORTIGUADORES: CONDICION, MONTURA, BUJES, ELEMENTOS DE SUJECION, POSICIONAMIENTO', 'val'=> $su?->amortiguadores],
      ...
      ['nom'=>'72','d'=>'','v'=>'✓','m'=>'','txt'=>'SALPICADERAS (LODERAS): CONDICION, ANCHO, ALTURA DESDE EL SUELO', 'val'=> $su?->salpicaderas],
      ['nom'=>'21','d'=>'','v'=>'✓','m'=>'','txt'=>'AMORTIGUADORES: CONDICION, MONTURA, BUJES, ELEMENTOS DE SUJECION, POSICIONAMIENTO', 'val'=> $amortTrasera2],
      ['nom'=>'57','d'=>'','v'=>'✓','m'=>'✓','txt'=>'BASTIDOR, CHASIS, LARGEROS', 'val'=> $su?->bastidor_largeros],
    ];
    $add($secciones, 'LUCES', $secLucesAutobus);
    $add($secciones, 'PARTE TRASERA', $secTraseraCabina);
    $add($secciones, 'LLANTAS DELANTERAS', $llDelanteras);
    $add($secciones, 'SUSPENSION DELANTERA', $secSuspDelantera);
    $add($secciones, 'SISTEMA DE DIRECCION', $secDireccionAutobus);
    ...
    $add($secciones, 'SUSPENSION TRASERA', $suspTraseraF21);
    $add($secciones, 'BATERIA', [
        ['nom'=>'54','d'=>'','v'=>'✓','m'=>'','txt'=>'ACUMULADOR DE BATERIA, SOPORTE, POSTES, CUBIERTA', 'val'=> $ch?->bateria],
    ]);
    $add($secciones, 'FRENOS NEUMATICOS', $secNeuAutobus);
...
} elseif ($tipoFormulario === 'F18_CAMION' || $tipoFormulario === 'F21_AUTOBUS') {
    // Oficial F-18 / F-21: solo XXXV y XXXVIII (sin XXXIX).
    $medicionesComplementarias = [
...
/* ── Pie de medición: solo llantas activas del tipo (p. ej. B2→6, B3→10) ──
...
<table class="chk" cellspacing="0">
  <tbody>
    <tr class="sec-row"><td colspan="8"><?= in_array($tipoFormulario, ['F18_CAMION', 'F21_AUTOBUS'], true)
