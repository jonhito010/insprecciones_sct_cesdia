<?php
/**
 * Lista de inspección — F-19 REV.01 Remolque/Semirremolque S-2, S-3 (Dompdf).
 * Plantilla exclusiva: no compartir con F-17/F-18/F-20/F-21.
 * Referencia: webroot/camposTXT/F-19_REMOQLUE.txt + f-19.pdf
 *
...
// Esta plantilla es solo F-19.
$tipoFormulario = 'F19_REMOLQUE';
$formularioNumero = 'F-19';
$formularioRev = 'F-19 REV.01';
$esCabina   = false;
$esRemolque = true;
$esDolly    = false;
$esCamion   = false;
...
/* Grupos F-19: plantilla oficial siempre 1/2…11/12 (40 filas).
 * S4 añade 13/14 y 15/16. Datos ausentes → celda en blanco. */
$maxLlTipo = (int)(\App\Validation\TipoVehiculoRequisitos::definicion($tipoVehCod)['llantas'] ?? 12);
$gruposLlantaF19 = [
    ['gn' => 1,  'titulo' => 'LLANTA 1/2 DE DISCO  ESPESOR MINIMO 1,6 mm', 'rin' => '3/4', 'rinEspesor' => true,  'pos' => null],
    ['gn' => 5,  'titulo' => 'LLANTA 5/6 INTERIOR ESPESOR MINIMO 1,6 mm',  'rin' => '5/6', 'rinEspesor' => true,  'pos' => null],
    ['gn' => 7,  'titulo' => 'LLANTA 7/8 EXTERIOR ESPESOR MINIMO 1,6 mm',   'rin' => '7/8', 'rinEspesor' => false, 'pos' => null],
    ['gn' => 9,  'titulo' => 'LLANTA 9/10 INTERIOR ESPESOR MINIMO 1,6 mm',  'rin' => '9/10', 'rinEspesor' => false, 'pos' => null],
    ['gn' => 11, 'titulo' => 'LLANTA 11/12 INTERIOR ESPESOR MINIMO 1,6 mm', 'rin' => '11/12', 'rinEspesor' => false, 'pos' => null],
];
if ($maxLlTipo >= 16 || $tipoVehCod === 'S4') {
    $gruposLlantaF19[] = ['gn' => 13, 'titulo' => 'LLANTA 13/14 INTERIOR ESPESOR MINIMO 1,6 mm', 'rin' => '13/14', 'rinEspesor' => false, 'pos' => null];
    $gruposLlantaF19[] = ['gn' => 15, 'titulo' => 'LLANTA 15/16 INTERIOR ESPESOR MINIMO 1,6 mm', 'rin' => '15/16', 'rinEspesor' => false, 'pos' => null];
}
...
// Oficial F-19: siempre imprime los 5 bloques del formato (aunque falte captura).
$llTraseras = [];
...
// Orden oficial F-19 REV.01 (F-19_REMOQLUE.txt / f-19.pdf). Solo este formato.
...
// F-19: sin mediciones XXXV/XXXVIII/XXXIX.
$medicionesComplementarias = [];

/* ── Pie de medición: solo llantas activas del tipo (S2/S3) ──
...
$getVarillaDisplay = static function (int $n, array $mmMap, array $resMap) use ($pdfEtiquetaCumple, $tipoVehCod): string {
    ...
    // Legacy motriz: datos en par 1-2 cuando ahora 1 y 2 van sueltas.
    if ($mm === '' && $rs === '' && ($n === 1 || $n === 2) && $par !== '1-2') {
        $mm = trim((string)($mmMap['1-2'] ?? ''));
        $rs = trim((string)($resMap['1-2'] ?? ''));
    }
    // Legacy remolque: datos en 1 y 2 sueltas cuando ahora van dobles 1-2.
    if ($mm === '' && $rs === '' && $par === '1-2') {
        $mm = trim((string)($mmMap['1'] ?? ''));
        $rs = trim((string)($resMap['1'] ?? ''));
        if ($mm === '' && $rs === '') {
            $mm = trim((string)($mmMap['2'] ?? ''));
            $rs = trim((string)($resMap['2'] ?? ''));
        }
    }
    if ($mm === '' && $rs === '') {
        return '';
...
/** Cámara pie F-19: por par (como Dolly) — una sola Abrazadera (mm) en todas las llantas. */
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Encabezado de columnas una sola vez (no <thead>: Dompdf no lo repite por hoja). */
  .chk tr.th-top td {
...
<!-- Metadatos oficiales F-19: sin kilometraje (arrastre). Folio prefijo A. -->
...
<?php
$mostrarVolanteHolgura = false; // F-19 no tiene dirección
?>
